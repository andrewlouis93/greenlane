...
var TimeDrag = React.createClass({
  ...
  initResizer: function() {
    ...
    var backButtonState = ScenicStore.getBackBtnState().states.length
      ? ScenicStore.getBackBtnState().states[ScenicStore.getBackBtnState().states.length - 1]
      : null;
  ...
  componentDidMount: function() {
    ...
    $('#resizer').draggable({
      ...
      stop: function(e, ui) {
        ...
        var cy = (top % grid_y);
        var new_top = (Math.abs(cy) + 0.5 * grid_y >= grid_y)
          ? (top - cy + (top / Math.abs(top)) * grid_y)
          : (top - cy);
  ...
  render: function() {
    return (
      <div className="row" id="timeSlider">
        ...
        <div className="row" id="drag-container">
          <div id="resize-cont">
            <div id="resizable-element"></div>
          </div>
          <div id="drag-cont">
            <div className="greenBoob" id="resizer"></div>
          </div>
          <p className="scaleG">faster</p>
        </div>

        <button className="btn-primary col s5 Green_Skip" onClick={this.handleSkip}>skip</button>
        <button autoFocus className="btn-secondary col s5 Time_Submit" onClick={Navigate.generateRoute}>map it</button>
      </div>
    );
  }
});
